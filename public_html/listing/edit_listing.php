...
// Fetch listing
$stmt = $conn->prepare("SELECT * FROM hostels WHERE id = ? AND user_id = ? AND role = ?");
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .sidebar {
      width:220px; background:var(--primary); color:#fff; flex-shrink:0;
      display:flex; flex-direction:column; position:fixed; top:0; left:0; bottom:0; padding:20px;
    ...
    .main { flex:1; margin-left:220px; padding:20px; transition: margin-left 0.3s ease-in-out; }
...
// File validation (≤5MB + jpg/jpeg/png only)
  ...
  fileInput.addEventListener('change', function () {
    ...
    for (const file of this.files) {
      if (!allowedTypes.includes(file.type)) {
        alert(`File "${file.name}" is not valid. Allowed: JPG, JPEG, PNG.`);
        this.value = '';
        return;
      }
